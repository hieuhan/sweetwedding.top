(function ($) {
  ...
  var main_menu = $(".gla_main_menu");
  $("#nav-sidebar, .gla_default_menu, .gla_main_menu_content_menu").onePageNav({
    ...
    easing: "swing",
    begin: function () {
      main_menu.next(".gla_main_menu_content").removeClass("active");
      main_menu
        .next()
  ...
  $(window).load(function () {
    ...
    var socket = io.connect("https://sweetwedding.top");
    socket.on("refresh wishes", function (res) {
      var el = $(".gla_timer"),
      ...
      if ($.isNumeric(num)) {
        ...
        el.text(total);
        if (showMsg.children(".gla_post_item").length > 2) {
          showMsg.find("div.gla_post_item:last").remove();
        }
        $(
          '<div class="gla_post_item" style="margin-bottom:10px;"><div class="gla_post_title"><h3>' +
          res.fullName +
          "</h3></div><p>" +
          res.messageContent +
          "</p></div>"
        ).insertAfter($("#show_messages > .title"));
      }
    });
